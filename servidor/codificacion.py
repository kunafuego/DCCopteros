...
CHUNK_SIZE_MENSAJE = p("chunk_size_mensajes")
CHUNK_SIZE_IMAGEN = p("chunk_size_imagenes")
"""
...
def decodificar_mensaje(mensaje):
    ...
    try:
        ...
        print("No pude decodificar a ", bytes_mensaje)
...
def codificar_imagen(ruta):
    ...
    #Como al codificar será la ruta, el color será el elemento número
    if "avatar-1" in ruta:
        bytes_color_imagen = (1).to_bytes(4, byteorder="big")
    elif "avatar-2" in ruta:
        bytes_color_imagen = (2).to_bytes(4, byteorder="big")
    elif "avatar-3" in ruta:
        bytes_color_imagen = (3).to_bytes(4, byteorder="big")
    elif "avatar-4" in ruta:
        bytes_color_imagen = (4).to_bytes(4, byteorder="big")
    bytes_contenido = b""
    numero_bloque = 0
